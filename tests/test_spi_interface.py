...
def test_as3925_spi_init_called_with_correct_args_and_kwargs(mocker):
    mock_init = mocker.patch.object(as3935.AS3935_SPI, "__init__", autospec=True, return_value=None)
    ...
    mock_init.assert_called_once_with(test_class, "spi", "cs", baudrate=1_000_000, interrupt_pin="pin")  
    

@pytest.mark.parametrize("spi, cs, cs_pin", [("spi1", "cs1", "cs_pin1"), ("spi2", "cs2", "cs_pin2")])
def test_spi_device_called_with_correct_args_and_kwargs(mocker, spi, cs, cs_pin):
    mock_spi_device = mocker.patch.object(as3935.spi_dev, "SPIDevice", autospec=True, return_value="SPIDevice")
    mock_digtalio_digitalinout = mocker.patch.object(as3935.digitalio, "DigitalInOut", autospec=True, return_value=cs_pin)
    mock_startup_checks = mocker.patch.object(as3935.AS3935_SPI, "_as3935_startup_checks", autospec=True, return_value=None)
    # Test call to SPIDevice
    test_class = as3935.AS3935_SPI(spi, cs, interrupt_pin="pin")
    mock_spi_device.assert_called_once_with(spi, cs_pin, baudrate=2_000_000, polarity=1, phase=0)
...
def test_spi_device_called_with_correct_baudrates(mocker):
    mock_spi_device = mocker.patch.object(as3935.spi_dev, "SPIDevice", autospec=True, return_value="SPI_Device")
    mock_digtalio_digitalinout = mocker.patch.object(as3935.digitalio, "DigitalInOut", autospec=True, return_value="cs")
    # Confirm SPIDevice is called with the default baudrate
    test_class = as3935.AS3935_SPI("spi", "cs", interrupt_pin="pin")
    mock_spi_device.assert_called_once_with("spi", "cs", baudrate=2_000_000, polarity=1, phase=0)
    # Confirm SPIDevice is called with the baudrate passed to __init__
    mock_spi_device.reset_mock()
    test_class = as3935.AS3935_SPI("spi", "cs", baudrate=1_000_000, interrupt_pin="pin")
    mock_spi_device.assert_called_once_with("spi", "cs", baudrate=1_000_000, polarity=1, phase=0)
